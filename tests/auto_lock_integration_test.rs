...
//!
//! These tests require the `gui` feature to be enabled.
...
async fn test_auto_lock_integration_flow() {
    ...
    assert!(
        ...
        "Should be ready for auto-lock after timeout"
    );
...
async fn test_activity_prevents_auto_lock_integration() {
    ...
    for _ in 0..4 {
        ...
        assert!(
            !auth.should_auto_lock().await,
            "Activity should prevent auto-lock"
        );
    }
...
async fn test_lock_stops_counter_integration() {
    ...
    assert_eq!(
        state.last_activity_secs, 0,
        "Counter should not increment while locked"
    );
}
...
async fn test_unlock_restarts_counter_integration() {
    ...
    assert_eq!(
        state_after_lock.last_activity_secs, 0,
        "Counter should be 0 after lock"
    );
...
async fn test_long_running_counter() {
    ...
    assert!(state.is_unlocked, "Vault should be unlocked");
    assert!(
        state.last_activity_secs >= 4 && state.last_activity_secs <= 6,
        "Counter should be around 5 seconds, got {}",
        state.last_activity_secs
    );
}
